# GuidedAnalysis Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace ScoreCard/TrustDimension/ContradictionAlert with a GuidedAnalysis component that shows raw facility data annotated with AI evidence chat bubbles, plus Flag-for-Review and Score Override modals.

**Architecture:** One new file `GuidedAnalysis.tsx` contains all sub-components. Bubble elements are appended imperatively to the GuidedAnalysis container div (which has `position: relative`) so they scroll with the content and can be positioned with `getBoundingClientRect()` math. Modals use `createPortal` to `document.body`. Workbench is simplified to notes-only; Flag and Override move to modals.

**Tech Stack:** React 19, Tailwind CSS (layout/typography), CSS variables `--fiq-*` and `--ga-*` (all theme-adaptive colors), `createPortal` for modals, imperative DOM for bubble elements.
...
| File | Action |
|---|---|
| `app/client/src/index.css` | Modify — add `--ga-*` CSS vars + `.ga-bubble` CSS + `@keyframes ga-pop` |
| `app/client/src/components/GuidedAnalysis.tsx` | Create — all types, helpers, sub-components, modals, main export |
| `app/client/src/components/Workbench.tsx` | Modify — notes only; remove signals prop, override, flag, shortlist |
| `app/client/src/App.tsx` | Modify — import GuidedAnalysis, remove ScoreCard import |
...
## Task 1: Add guided analysis CSS to `index.css`

**Files:**
- Modify: `app/client/src/index.css`

- [ ] **Step 1: Add `--ga-*` CSS variables to the existing `:root` and `[data-theme="light"]` blocks**

Read `app/client/src/index.css` first, then in the `:root` block (the FacilityIQ theme block at the top, NOT the commented AppKit block), append these lines before the closing `}`:

```css
  /* GuidedAnalysis — highlight + bubble colors (theme-adaptive) */
  --ga-hl-e-bg: rgba(99,179,237,0.15);
  --ga-hl-e-color: #3730a3;
  --ga-hl-e-underline: rgba(99,179,237,0.5);
  --ga-hl-c-bg: rgba(251,191,36,0.15);
  --ga-hl-c-color: #92400e;
  --ga-hl-c-underline: rgba(251,191,36,0.55);
  --ga-bubble-e-bg: #ffffff;
  --ga-bubble-e-border: #c7d2fe;
  --ga-bubble-e-tail: #ffffff;
  --ga-bubble-c-bg: #fffbeb;
  --ga-bubble-c-border: #fde68a;
  --ga-bubble-c-tail: #fffbeb;
```

In the `[data-theme="light"]` block, append before its closing `}`:

```css
  --ga-hl-e-bg: rgba(79,70,229,0.1);
  --ga-hl-e-color: #3730a3;
  --ga-hl-e-underline: rgba(79,70,229,0.4);
  --ga-hl-c-bg: rgba(180,83,9,0.1);
  --ga-hl-c-color: #92400e;
  --ga-hl-c-underline: rgba(180,83,9,0.4);
  --ga-bubble-e-bg: #ffffff;
  --ga-bubble-e-border: #c7d2fe;
  --ga-bubble-e-tail: #ffffff;
  --ga-bubble-c-bg: #fffbeb;
...
- [ ] **Step 2: Add bubble component CSS and keyframe after the scrollbar rules**

After the `html { scrollbar-width: ... }` line in `app/client/src/index.css`, add:

```css

/* ── GuidedAnalysis bubble system ── */
.ga-bubble { border-radius: 12px; }
.ga-bubble-e {
  background: var(--ga-bubble-e-bg);
  border: 1.5px solid var(--ga-bubble-e-border);
  filter: drop-shadow(0 6px 16px rgba(79,70,229,.15)) drop-shadow(0 2px 4px rgba(0,0,0,.07));
}
.ga-bubble-c {
  background: var(--ga-bubble-c-bg);
  border: 1.5px solid var(--ga-bubble-c-border);
  filter: drop-shadow(0 6px 16px rgba(245,158,11,.18)) drop-shadow(0 2px 4px rgba(0,0,0,.07));
}
.ga-bubble-body {
  padding: 10px 13px; font-size: 11px; line-height: 1.55;
  display: flex; align-items: flex-start; gap: 7px;
  color: var(--fiq-text-muted);
}
.ga-bubble-icon { font-size: 12px; flex-shrink: 0; }
.ga-bubble-text { flex: 1; }
.ga-score-chip {
  display: inline-block; font-size: 9px; font-weight: 800;
  padding: 1px 6px; border-radius: 4px; margin-left: 4px; vertical-align: middle;
}
.ga-score-high { background: rgba(74,222,128,.15); color: var(--fiq-trust-high); border: 1px solid rgba(74,222,128,.3); }
.ga-score-med  { background: rgba(251,191,36,.15);  color: var(--fiq-trust-med);  border: 1px solid rgba(251,191,36,.3); }
.ga-score-low  { background: rgba(248,113,113,.15); color: var(--fiq-trust-low);  border: 1px solid rgba(248,113,113,.3); }
.ga-tail { position: absolute; width: 0; height: 0; }
@keyframes ga-pop {
...
git commit -m "feat: add guided analysis CSS vars and bubble styles"
```

---

## Task 2: Create `GuidedAnalysis.tsx`

**Files:**
- Create: `app/client/src/components/GuidedAnalysis.tsx`

- [ ] **Step 1: Create the file**

Create `app/client/src/components/GuidedAnalysis.tsx` with the following complete content:
...
Expected: errors about Workbench receiving an unknown `signals` prop (the current Workbench requires it). That will be fixed in Task 3. Any other TypeScript errors must be fixed before proceeding.

- [ ] **Step 3: Commit**

```bash
cd /Users/brett.plemons/Documents/Development/facilityiq
git add app/client/src/components/GuidedAnalysis.tsx
git commit -m "feat: add GuidedAnalysis component with evidence bubbles, flag modal, and score override"
```

---

## Task 3: Simplify `Workbench.tsx` + update `App.tsx` + delete old files

**Files:**
- Modify: `app/client/src/components/Workbench.tsx`
- Modify: `app/client/src/App.tsx`
- Delete: `app/client/src/components/ScoreCard.tsx`
- Delete: `app/client/src/components/TrustDimension.tsx`
- Delete: `app/client/src/components/ContradictionAlert.tsx`

- [ ] **Step 1: Replace `Workbench.tsx` with notes-only version**

Read the current `app/client/src/components/Workbench.tsx`, then replace the entire file:
...
- [ ] **Step 2: Update `App.tsx` — swap ScoreCard for GuidedAnalysis**

Read the current `app/client/src/App.tsx`, then make two edits:

**Edit 1** — change the import:
```tsx
// Remove:
...
**Edit 2** — change the JSX render (find `<ScoreCard` and replace with `<GuidedAnalysis`):
```tsx
// Remove:
...
- [ ] **Step 3: Delete the three retired files**

```bash
rm /Users/brett.plemons/Documents/Development/facilityiq/app/client/src/components/ScoreCard.tsx
rm /Users/brett.plemons/Documents/Development/facilityiq/app/client/src/components/TrustDimension.tsx
rm /Users/brett.plemons/Documents/Development/facilityiq/app/client/src/components/ContradictionAlert.tsx
```
...
```bash
cd /Users/brett.plemons/Documents/Development/facilityiq
git add app/client/src/components/Workbench.tsx app/client/src/App.tsx
git rm app/client/src/components/ScoreCard.tsx app/client/src/components/TrustDimension.tsx app/client/src/components/ContradictionAlert.tsx
git commit -m "feat: wire GuidedAnalysis into app, simplify Workbench to notes-only, remove old ScoreCard components"
```
...
sleep 1
cd /Users/brett.plemons/Documents/Development/facilityiq/app && npm run dev > /tmp/facilityiq-dev.log 2>&1 &
echo $! > /tmp/facilityiq.pid
for i in $(seq 1 30); do curl -sf http://localhost:8000 >/dev/null 2>&1 && echo "ready" && break || sleep 1; done
```

- [ ] **Step 2: Open and verify the new ScoreCard (GuidedAnalysis)**

Open `http://localhost:8000`. Click any facility. Confirm:

1. The right panel shows the new layout: facility name, overall score top-right, ⚑ Flag for Review button
2. Four Trust Dimension cards with scores/bars and ✏ icons on scoreable dimensions (no ✏ on Completeness NO DATA card)
3. "Facility Data" section with grouped rows: Identity, Clinical, Capacity
4. Highlighted spans (indigo for evidence, amber for contradiction) are visible in Clinical fields
5. Hover a highlighted span → chat bubble appears above it with a CSS triangle tail pointing down toward the span
6. Click "◈ Show all evidence" → all bubbles appear, each positioned to avoid overlap, tails pointing at their spans
7. Click "◉ Hide evidence" → all bubbles disappear completely
8. Click ⚑ Flag for Review → modal opens with textarea and Cancel/⚑ Flag buttons; clicking outside closes it
9. Click ✏ on a dimension card → override modal opens with that dimension pre-filled; Cancel works; fill score + reason → Apply Override saves
10. Analyst Notes section at bottom: type a note, Save Note → reopen same facility → note persists
11. Switch to light mode (☀ in sidebar) → highlights, bubbles, and text all adapt

- [ ] **Step 3: Stop server and final commit**
...
git add -A
git status
# Should be clean — all changes committed in tasks 1–3
```

If any stray uncommitted files exist:
```bash
git commit -m "feat: guided analysis implementation complete"
```
